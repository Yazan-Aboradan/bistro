...
import 'package:bistro/app/global_pages/blurred_page.dart';
    ...
    return Center(
      child: AlertDialog(
        ...
        content: AppContainerWidget(
          width: 400,
          height: 460 + calculateTextHeight(product.product.description + product.product.ingredients.map((i) => i.name).join('  -  '),400),
          child: Stack(
            children: [
              AppColumnWidget(
                ...
                children: [
                  Expanded(
                    ...
                    child: AppImageWidget(
                      path: product.product.imagePath == '' ? product.product.category.greenSvgPath : product.product.imagePath,
                      width: 400,
                    ),
                  ),
                  Expanded(
                    ...
                    child: AppColumnWidget(
                      ...
                      children: [
                        ...
                        if(product.product.ingredients!=null && product.product.ingredients.length!=0)...[
                          Gap(12),
                          AppTextWidget(
                            textAlign: TextAlign.center,
                            text: product.product.ingredients.map((i) => i.name).join('  -  '),
    ...
    return AppRowWidget(
      ...
      children: [
        Expanded(
          child: Center(
            child: AppTextWidget(
              text: '$oldPrice ${context.localizations.syp}',
              style: oldPriceStyle ??
                  TextStyle(
                    fontSize: AppDimensions.fontSize20,
                    color: Colors.grey,
        ...
        Expanded(
          child: Center(
            child: AppTextWidget(
              text: '$newPrice ${context.localizations.syp}',
              style: newPriceStyle ??
                  TextStyle(
...
class FeaturedProduct {
  ...
  factory FeaturedProduct.fromJson(Map<String, dynamic> json) {
    if (json['product'] == null) {
      throw ArgumentError('Product cannot be null in FeaturedProduct');
    }
    ...
    return FeaturedProduct(
      ...
      offerExpireDate: json['offerExpireDate'] as String?,
  ...
  Map<String, dynamic> toJson() {
    return {
      'id': id,
      'product': product.toJson(), // non-nullable, no '?'
      'isOffer': isOffer,
